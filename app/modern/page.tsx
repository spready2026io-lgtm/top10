...
function conviction(coverage: number): { label: string; color: string } {
  ...
  if (coverage >= 0.35) return { label: 'Medium', color: 'var(--ss-amber-text)' };
  return { label: 'Low', color: 'var(--ss-text)' };
}

// ── Explore pills → real routes ───────────────────────────────────────────────
const EXPLORE = [
  { label: 'Build Portfolio', href: '/modern/portfolio' },
  { label: 'Conviction Board', href: '/modern/conviction' },
  { label: 'World Markets', href: '/modern/markets' },
  { label: 'Ask Tony', href: '/modern/ask' },
  { label: 'ETF Universe', href: '/modern/universe' },
];
    ...
    <div style={{ minHeight: '100vh', background: 'var(--ss-page)', color: 'var(--ss-ink)' }}>
      ...
      <header
        style={{
          ...
          background: 'var(--ss-nav-bg)', backdropFilter: 'blur(12px)',
      ...
      >
        <div className="ss-nav-bar" style={{ maxWidth: 1200, margin: '0 auto', height: 70, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          <nav className="ss-nav-row">
            ...
            <Link href="/modern/contact" className="ss-link-muted ss-nav-links-secondary" style={{ fontSize: 15, fontWeight: 500 }}>Contact</Link>
            {/* Back to the default surface. */}
            <Link href="/" className="ss-link-muted ss-nav-links-secondary" style={{ fontSize: 15, fontWeight: 500 }}>← Classic</Link>
            <ThemeToggle />
            <Link href="/modern/rankings" className="ss-nav-cta" style={{ fontSize: 14, fontWeight: 700, color: '#fff', background: GREEN, padding: '11px 20px', borderRadius: 999 }}>See rankings</Link>
          </nav>
        </div>
      </header>
      ...
      <section style={{ position: 'relative', overflow: 'hidden' }}>
        ...
        <div className="ss-hero-grid" style={{ position: 'relative', maxWidth: 1200, margin: '0 auto', padding: '80px 32px 56px' }}>
          <div>
            <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: 'var(--ss-green-tint)', border: '1px solid var(--ss-green-tint-border)', color: 'var(--ss-green-text)', fontSize: 12, fontWeight: 700, letterSpacing: 0.4, padding: '7px 13px', borderRadius: 999, marginBottom: 26 }}>
              <span style={{ width: 6, height: 6, borderRadius: '50%', background: '#10B981' }} />UPDATED DAILY · NO LOGIN
            </div>
            <h1 style={{ fontSize: 62, lineHeight: 1.02, fontWeight: 800, letterSpacing: '-2px', color: 'var(--ss-ink)', margin: '0 0 22px' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 24, flexWrap: 'wrap' }}>
              <Link href="/modern/rankings" style={{ display: 'inline-flex', alignItems: 'center', gap: 10, fontSize: 16, fontWeight: 700, color: '#fff', background: GREEN, padding: '16px 28px', borderRadius: 14 }}>See the rankings <span style={{ fontSize: 18 }}>↓</span></Link>
              <a href="#how" style={{ display: 'inline-flex', alignItems: 'center', fontSize: 16, fontWeight: 600, color: 'var(--ss-ink)', background: 'var(--ss-card)', border: '1px solid var(--ss-border-strong)', padding: '16px 26px', borderRadius: 14 }}>Read the guide</a>
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 36, paddingTop: 30, borderTop: '1px solid var(--ss-border)', flexWrap: 'wrap' }}>
          ...
          <div>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
              <span style={{ ...mono, fontSize: 11, letterSpacing: 1, color: 'var(--ss-amber-text)', background: 'var(--ss-amber-bg)', border: '1px solid var(--ss-amber-border)', padding: '4px 10px', borderRadius: 6 }}>EXAMPLE TILE</span>
              <span style={{ fontSize: 13, color: 'var(--ss-muted)' }}>Broad Tech · #1 conviction</span>
            </div>
            <div style={{ background: 'var(--ss-card)', border: '1px solid var(--ss-border)', borderRadius: 18, padding: 24, boxShadow: '0 18px 40px rgba(11,18,32,0.08)' }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 13 }}>
                  <StockAvatar ticker={ex.ticker} size={42} radius={12} bg="linear-gradient(135deg,var(--ss-avatar-blue-1),var(--ss-avatar-blue-2))" textColor="var(--ss-blue)" fontSize={14} />
                  <div>
                    <div style={{ fontSize: 17, fontWeight: 700, color: 'var(--ss-ink)' }}>{ex.name}</div>
                    <div style={{ fontSize: 13, color: 'var(--ss-muted)' }}>{ex.ticker} · Broad Tech</div>
                  </div>
                </div>
                <div style={{ textAlign: 'right' }}>
                  <div style={{ ...mono, fontSize: 22, fontWeight: 700, color: 'var(--ss-ink)' }}>${ex.price.toFixed(2)}</div>
                  <div style={{ ...mono, fontSize: 13, fontWeight: 700, color: moveColor(ex.weeklyChange) }}>{signed(ex.weeklyChange)} 1W</div>
                </div>
              </div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                <Inset label="Coverage" value={<><span>{ex.easyScore}</span><span style={{ color: 'var(--ss-faint)', fontSize: 14 }}>/{exN} ETFs</span></>} />
                <Inset label="Avg weight" value={`${ex.avgWeight?.toFixed(1) ?? '—'}%`} />
                <Inset label="Velocity 1W" value={exVel == null ? 'New' : signed(exVel)} color={exVel == null ? 'var(--ss-muted)' : moveColor(exVel)} />
                <Inset label="Conviction" value={exConv.label} color={exConv.color} />
              </div>
            </div>
            <div style={{ fontSize: 13, color: 'var(--ss-muted)', marginTop: 12, textAlign: 'center' }}>Free. No login. Updated daily.</div>
          </div>
        </div>
        ...
        <div style={{ maxWidth: 1200, margin: '0 auto', padding: '0 32px 64px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
            <Link href="/modern/rankings?theme=all" style={{ display: 'inline-flex', alignItems: 'center', gap: 7, fontSize: 14, fontWeight: 700, color: '#fff', background: GREEN, padding: '10px 16px', borderRadius: 999 }}>🔥 All-Theme Top 10</Link>
            {EXPLORE.map((p) => (
      ...
      {/* ============ HOW IT WORKS ============ */}
      <section id="how" style={{ background: 'var(--ss-card)', borderTop: '1px solid var(--ss-track)', borderBottom: '1px solid var(--ss-track)' }}>
        <div style={{ maxWidth: 1200, margin: '0 auto', padding: '72px 32px' }}>
          <div style={{ maxWidth: 640, marginBottom: 44 }}>
            ...
            <h2 style={{ fontSize: 38, fontWeight: 800, letterSpacing: '-1px', color: 'var(--ss-ink)', margin: '0 0 12px' }}>Three steps, refreshed every day</h2>
            <p style={{ fontSize: 18, lineHeight: 1.55, color: 'var(--ss-text)', margin: 0 }}>No opinions, no noise — just what the managers actually hold, scored and ranked.</p>
          </div>
      ...
      <section id="leaderboard" style={{ maxWidth: 1200, margin: '0 auto', padding: '72px 32px' }}>
        <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', flexWrap: 'wrap', gap: 16, marginBottom: 28 }}>
          ...
          <Link href="/modern/rankings?theme=all" style={{ fontSize: 15, fontWeight: 700, color: GREEN }}>View full rankings →</Link>
        </div>
        ...
        <div className="ss-lb-scroll" style={{ background: 'var(--ss-card)', border: '1px solid var(--ss-border)', borderRadius: 18, overflow: 'hidden', boxShadow: '0 12px 30px rgba(11,18,32,0.05)' }}>
          <div className="ss-lb-grid" style={{ padding: '14px 24px', background: 'var(--ss-inset)', borderBottom: '1px solid var(--ss-border)', fontSize: 11, letterSpacing: 0.8, color: 'var(--ss-muted)', textTransform: 'uppercase', fontWeight: 700 }}>
          ...
          {rows.map((r, i) => {
            ...
            return (
              <div key={r.ticker} className="ss-lb-grid" style={{ alignItems: 'center', padding: '16px 24px', borderBottom: i === rows.length - 1 ? 'none' : '1px solid var(--ss-track)' }}>
                <div style={{ ...mono, fontSize: 16, fontWeight: 700, color: i === 0 ? GREEN : 'var(--ss-muted)' }}>{i + 1}</div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                  <StockAvatar ticker={r.ticker} size={38} radius={10} />
                  <div>
                    <div style={{ fontSize: 16, fontWeight: 700, color: 'var(--ss-ink)' }}>{r.name}</div>
                    <div style={{ fontSize: 13, color: 'var(--ss-muted)' }}>{r.themeCount} themes</div>
                  </div>
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  <div style={{ width: 88, height: 7, borderRadius: 999, background: 'var(--ss-track)', overflow: 'hidden' }}>
                    <div style={{ height: '100%', width: `${pct}%`, background: GREEN, borderRadius: 999 }} />
                  </div>
                  <span style={{ ...mono, fontSize: 13, color: 'var(--ss-text)' }}>{r.themeCount}/{THEME_COUNT}</span>
                </div>
                <div style={{ ...mono, textAlign: 'right', fontSize: 15, fontWeight: 700, color: 'var(--ss-ink)' }}>{r.avgProScore.toFixed(2)}%</div>
        ...
        <div style={{ maxWidth: 1200, margin: '0 auto', padding: '48px 32px' }}>
          <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', flexWrap: 'wrap', gap: 32, marginBottom: 36 }}>
            ...
            <div style={{ display: 'flex', gap: 64, flexWrap: 'wrap' }}>
              <FooterCol title="Product" links={[['Rankings', '/modern/rankings'], ['Build Portfolio', '/modern/portfolio'], ['World Markets', '/modern/markets'], ['Ask Tony', '/modern/ask']]} />
              <FooterCol title="Company" links={[['About', '/modern/about'], ['Contact', '/modern/contact'], ['How it works', '#how'], ['ETF Universe', '/modern/universe']]} />
            </div>
          </div>
...
function Stat({ value, label }: { value: string; label: string }) {
  return (
    <div>
      <div style={{ ...mono, fontSize: 28, fontWeight: 700, color: 'var(--ss-ink)' }}>{value}</div>
      <div style={{ fontSize: 12, letterSpacing: 0.8, color: 'var(--ss-muted)', textTransform: 'uppercase', marginTop: 3 }}>{label}</div>
    </div>
  );
}
function Divider() {
  return <div style={{ width: 1, height: 34, background: 'var(--ss-border-strong)' }} />;
}
function Inset({ label, value, color = 'var(--ss-ink)' }: { label: string; value: React.ReactNode; color?: string }) {
  return (
    <div style={{ background: 'var(--ss-inset)', borderRadius: 12, padding: '14px 16px' }}>
      <div style={{ fontSize: 11, letterSpacing: 0.6, color: 'var(--ss-muted)', textTransform: 'uppercase', marginBottom: 6 }}>{label}</div>
...
function Step({ n, title, body }: { n: string; title: string; body: string }) {
  return (
    <div style={{ background: 'var(--ss-page)', border: '1px solid var(--ss-border)', borderRadius: 16, padding: 30 }}>
      <div style={{ ...mono, width: 44, height: 44, borderRadius: 12, background: 'var(--ss-green-tint)', color: GREEN, display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700, fontSize: 18, marginBottom: 18 }}>{n}</div>
      <div style={{ fontSize: 20, fontWeight: 700, color: 'var(--ss-ink)', marginBottom: 8 }}>{title}</div>
      <p style={{ fontSize: 16, lineHeight: 1.55, color: 'var(--ss-text)', margin: 0 }}>{body}</p>
    </div>
  );
}
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      ...
      {links.map(([label, href]) =>
        href.startsWith('#')
          ? <a key={label} href={href} style={{ fontSize: 15, color: 'var(--ss-text)' }}>{label}</a>
