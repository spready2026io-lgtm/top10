...
export default function AskTonyButton() {
  ...
  // The FAB is rendered by the root layout, so it sits on both surfaces. Keep
  // the visitor on the one they are already browsing.
  const modern = pathname === '/modern' || pathname.startsWith('/modern/');
  const askHref = modern ? '/modern/ask' : '/ask';
  if (pathname === askHref) return null;
