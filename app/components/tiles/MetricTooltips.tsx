  ...
  return (
    <div
      style={{
        ...
        width, textAlign: 'left', background: 'var(--ss-card)', border: '1px solid var(--ss-border)',
...
function TipTitle({ children }: { children: React.ReactNode }) {
  return <p style={{ fontSize: 11, fontWeight: 700, letterSpacing: 0.4, color: 'var(--ss-muted)', textTransform: 'uppercase', margin: '0 0 8px' }}>{children}</p>;
}
function TipRule() {
  return <div style={{ borderTop: '1px solid var(--ss-track)', margin: '8px 0 6px' }} />;
}
function Row({ l, r, mono, rColor = 'var(--ss-ink)' }: { l: string; r: string; mono?: boolean; rColor?: string }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12 }}>
      <span style={{ fontSize: 12, color: 'var(--ss-text)', fontFamily: mono ? MONO : undefined, fontWeight: mono ? 700 : 400 }}>{l}</span>
...
export function AvgWtTip({ equity, themeEtfs, align = 'right' }: { equity: Equity; themeEtfs: string[]; align?: 'left' | 'right' }) {
  ...
  return (
    <Tip align={align}>
      ...
      <p style={{ fontSize: 11, color: 'var(--ss-muted)', margin: '6px 0 0' }}>Weight Score = avg weight × coverage</p>
    </Tip>
  );
}
...
export function VelocityTip({ equity, align = 'right' }: { equity: Equity; align?: 'left' | 'right' }) {
  ...
  if (v1w == null) {
    return (
      <Tip align={align} width={190}>
        ...
        <p style={{ fontSize: 12, color: 'var(--ss-text)', margin: 0 }}>No 1-week history yet — new to the ranking. It populates as conviction history accrues.</p>
      </Tip>
    );
  }
  ...
  return (
    <Tip align={align}>
      ...
      <p style={{ fontSize: 11, color: 'var(--ss-muted)', margin: '6px 0 0' }}>Velocity = (now ÷ then − 1) × 100</p>
    </Tip>
  );
}
