...
function PerfChart({ theme, d, period, setPeriod }: { theme: Theme; d: { top10: number[]; spy: number[]; top10Return: number; spyReturn: number; xLabels: string[] }; period: ChartPeriod; setPeriod: (p: ChartPeriod) => void }) {
  ...
  return (
    <div style={{ background: '#fff', border: '1px solid #e6e9ef', borderRadius: 16, padding: '16px 18px', boxShadow: '0 8px 22px rgba(11,18,32,0.05)', display: 'flex', flexDirection: 'column' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 12, flexWrap: 'wrap' }}>
        <span style={{ fontSize: 15, fontWeight: 700, color: '#0B1220' }}>{theme} Top 10 vs S&amp;P 500</span>
        <span style={{ fontFamily: MONO, fontSize: 12, fontWeight: 700, padding: '3px 10px', borderRadius: 999, color: deltaPos ? '#0a7350' : AMBER, background: deltaPos ? '#e7f7f0' : '#fcf3e1', border: `1px solid ${deltaPos ? '#b8e6d3' : '#f0d9a8'}` }}>
          {deltaPos ? '▲' : '▼'} {Math.abs(delta).toFixed(1)}% vs index
        </span>
      </div>

      <svg viewBox={`0 0 ${VW} ${VH}`} width="100%" style={{ display: 'block' }}>
        {yTicks.map((t, i) => <line key={i} x1={padL} y1={toY(t)} x2={VW - padR} y2={toY(t)} stroke="#eef1f5" strokeWidth="1" />)}
        {zeroInRange && <line x1={padL} y1={toY(100)} x2={VW - padR} y2={toY(100)} stroke="#cbd5e1" strokeWidth="1.5" strokeDasharray="5 3" />}
        {yTicks.map((t, i) => <text key={`l${i}`} x={padL - 8} y={toY(t) + 3} textAnchor="end" fontSize="11" fill="#a4adba" fontFamily="var(--font-mono-brand), monospace">{fmtY(t)}</text>)}
        ...
        <circle cx={toX(d.top10.length - 1, d.top10.length)} cy={toY(d.top10[d.top10.length - 1])} r="4" fill={GREEN} stroke="#fff" strokeWidth="2" />
        {xl.map((lab, i) => <text key={`x${i}`} x={padL + (i / (xl.length - 1)) * chartW} y={VH - 8} textAnchor="middle" fontSize="11" fill="#a4adba">{lab}</text>)}
      </svg>
      ...
      <div style={{ display: 'flex', justifyContent: 'center', marginTop: 8 }}>
        <div style={{ display: 'inline-flex', gap: 2, background: '#f4f6f9', border: '1px solid #eaeef2', borderRadius: 10, padding: 3 }}>
          {RANGES.map((r) => {
            ...
            return <button key={r} onClick={() => setPeriod(r)} style={{ fontFamily: MONO, fontSize: 12, fontWeight: active ? 700 : 600, color: active ? '#0B1220' : '#8a94a3', background: active ? '#fff' : 'transparent', padding: '6px 12px', borderRadius: 7, border: 'none', cursor: 'pointer', boxShadow: active ? '0 1px 2px rgba(11,18,32,0.08)' : 'none' }}>{r}</button>;
          })}
        </div>
      </div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 20, marginTop: 10, flexWrap: 'wrap' }}>
        ...
        <span style={{ marginLeft: 'auto', fontSize: 12, color: '#a4adba' }}>indicative</span>
      </div>
    </div>
  );
}
...
function Legend({ color, label, value }: { color: string; label: string; value: number }) {
  return (
    <span style={{ display: 'inline-flex', alignItems: 'center', gap: 7, fontSize: 13, color: '#55606e' }}>
  ...
  return (
    <div style={{ background: '#fff', border: '1px solid #e6e9ef', borderRadius: 16, padding: '16px 18px', boxShadow: '0 8px 22px rgba(11,18,32,0.05)', display: 'flex', flexDirection: 'column' }}>
      <div style={{ marginBottom: 12 }}>
        <div style={{ fontSize: 15, fontWeight: 700, color: '#0B1220' }}>{theme} ETFs</div>
        <div style={{ fontSize: 12, color: '#8a94a3', marginTop: 3, display: 'flex', alignItems: 'center', gap: 5, flexWrap: 'wrap' }}>
          {n} tracked · Ranked by
          <span style={{ fontFamily: MONO, fontSize: 11, fontWeight: 700, color: '#0a7350', background: '#e7f7f0', border: '1px solid #b8e6d3', padding: '1px 6px', borderRadius: 5 }}>{period}</span>
          return
        </div>
      </div>
        ...
        {visible.map(({ ticker, ret }) => {
          ...
          return (
            <div key={ticker}>
              ...
              {info && (
                <div style={{ fontSize: 11, color: '#8a94a3', marginBottom: 5, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
      ...
      {rows.length > 4 && (
        <button onClick={() => setShowAll((v) => !v)} style={{ marginTop: 12, fontSize: 12, fontWeight: 600, color: '#8a94a3', background: 'none', border: 'none', cursor: 'pointer', textAlign: 'center' }}>
